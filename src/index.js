...
const app = {
  ...
  readPromise(service, characteristic) {
    return new Promise(function (resolve, reject) {
  ...
  sendCmd(deviceId, data, success, failure) {
  ...
  sendSpp(deviceId, data, success, failure) {
    // resultDiv.innerHTML = resultDiv.innerHTML + "sendSpp "+data.byteLength  + "<br/>";
    // resultDiv.scrollTop = resultDiv.scrollHeight;
  ...
  onError(e) {
    app.log(`ERROR: ${e.message}<br/><pre>${e.stack()}</pre>`); // real apps should use notification.alert
  }
};
